...
> - **Tavily testé en réel** (clé ajoutée le 18/09/2026) : l'API répond en 1–2 s et renvoie des sites marocains pertinents, mais surtout des **pages de listing d'agrégateurs** (datao.ma, marchefacile.ma, j360.info, e-marchespublics.com), pas des avis individuels ⇒ l'extracteur rend `is_tender=false` ; ces agrégateurs sont à configurer comme sources `portal` (`listing_paths` + `link_pattern`) pour suivre les liens vers les avis. `marchespublics.gov.ma` n'est **pas indexé** par Tavily (`include_domains` renvoie du bruit à score < 0,05, désormais filtré par `TavilySearch.min_score = 0.1`) : le portail officiel exige son connecteur dédié (Phase 11). Garde-fous ajoutés : `OpenAILLM.structured` borné à 4 096 jetons (un listing faisait générer 32 768 jetons en 4 min 45 s), troncature ⇒ `LLMError` sans retry, délai client 120 s.
...
> **Écarts constatés à l'exécution (18/09/2026) — à respecter dans les phases suivantes :**
> - Migration `9e3f4a5b6c7d_scoring` (convention `<rev>_<slug>.py`, pas `0007_`). `TenderDecision.decided_by` / `TenderStatusHistory.changed_by` = **email lisible** (règle Phase 3), l'`user_id` va dans l'audit ; horodatages `computed_at` / `decided_at` / `changed_at` **côté Python** (`now()` PostgreSQL est constant par transaction, l'ordre de l'historique serait ambigu). `Tender.score_total` est une propriété (relation `score` à charger avec `selectinload`) ; tri `score` / `-score` par jointure `tender_scores`.
> - `services/scoring.py` expose aussi `LABELS` (libellés FR des 8 clés) et `ScoreResult.summary()` (« Secteur 100/100 · … »), réutilisés par le prompt et le repli RB-004. Rapprochements via `norm_text` + `token_set_ratio ≥ 85` ; `experience` accepte aussi un secteur de projet cité dans le texte de l'AO ; `budget` = `budget_max or budget_min` contre la fourchette du profil.
> - `ScoreService(db, llm=None)` : l'enfilement des jobs `calculate_match_score` se fait dans la **tâche `search_tenders`** après la passe (`_enqueue_scoring`, une fois par fiche créée ou fusionnée, `job.result["scoring_jobs"]`) — pas dans `IngestService` : `JobService.enqueue` commite. `NOUVEAU → A_ANALYSER` au-delà de `relevance_threshold` est historisé (auteur `None` = système) sans passer par `transition()`. Testé en réel : gpt-4.1-mini, ≈ 1 100 jetons, 5–7 s, justification cohérente ; les fiches trouvées par Tavily sont scorées automatiquement.
> - `services/tender_status.py` : `transition(db, tender, to, *, comment, user)` et `decide(db, tender, decision, *, reason, user)` reçoivent l'objet `User` (pas `user_id`) ; `decide` passe implicitement par `A_ANALYSER` depuis `NOUVEAU` / `NO_GO` et refuse une décision égale au statut courant (« déjà en NO_GO »). Le routeur `api/v1/scoring.py` (préfixe `/tenders`) est monté **avant** `tenders.router` : `/tenders/kanban` serait sinon capturé par `/tenders/{tender_id}` (422). Kanban : actives hors statuts clos + closes (`GAGNE` / `PERDU` / `ARCHIVE`) dont `updated_at` < 90 j, meilleur score d'abord.
> - Frontend : `lib/tenders.ts` porte le **miroir** de `TRANSITIONS` (`nextStatuses`, `canDecide`) et `scoreLevel` (≥ 70 vert `success`, 40–69 jaune `warning`, < 40 `destructive` — pas d'orange dans la charte) ; `useTenderScore` renvoie `null` sur 404 `score_missing`. Le titre de la liste mène à `/tenders/[id]`, l'annonce externe a son propre lien ; bouton « Pipeline » ⇄ « Vue liste » (pas d'entrée de menu : `isActivePath` confondrait `/tenders` et `/tenders/kanban`). `DropdownMenuLabel` doit être dans un `DropdownMenuGroup` (Base UI). Dans les sélecteurs Playwright, échapper le `/` des regex (`27\/2026`).
...
> **Écarts constatés à l'exécution (20/09/2026) — à respecter dans les phases suivantes :**
> - `TenderDocumentService(db, storage, http: httpx.Client | None)` ; le client de téléchargement vient de `deps.get_download_client()` (`_download_client_override`, 404 simulé en test). `download()` ne lève jamais (`done` | `failed` + `error`), type = en-tête `Content-Type` s'il est autorisé sinon signature binaire (`filetype`), plafond `max_upload_mb`, clé `tenders/{tender_id}/xx/<sha>.<ext>`. Le job `download_tender_documents` commite après chaque pièce. `document_name` vit dans `services/normalize` (une seule définition). Migrations : `af4b5c6d7e8f_document_chunks` (+ `company_documents.page_count`), `b05c6d7e8f9a_tender_analysis`.
> - Extraction : `extract()` rend une liste (un ZIP ⇒ plusieurs documents, `metadata.inner_filename`, un niveau de récursion ; membres ignorés dans `metadata.skipped`, illisibles dans `metadata.failed`, chemin `../` ⇒ erreur). PDF scanné ⇒ `ExtractionError("PDF sans texte (scan) …")`. Itérer PyMuPDF par `doc[i]` sur `range(doc.page_count)` (mypy ne type pas `for page in doc`).
> - Découpage : `chunk_pages` coupe paragraphe › phrase › espace, recouvrement dans la page seulement, `detect_section` (« Article n », « 3.2 … », court, sans point final) ouvre un nouveau morceau. `DocumentChunk.meta` est la colonne `metadata` (nom réservé par SQLAlchemy). `IndexingService.index()` ne lève pas : `failed` + `last_error`, le job `index_document` rend `{chunks: 0, error}` et reste `done`. `extracted_text` = pages séparées par `\f` (`build_corpus` reconstruit `=== fichier — page n ===`).
> - Analyse : `AnalysisService.analyze` lève `AppError 422 no_documents` sans pièce exploitable (le job `analyze_tender` échoue lisiblement) ; les erreurs LLM remontent (pas de repli : l'analyse EST le résultat). La fiche est complétée sans écrasement (résumé toujours, échéance/référence/organisme seulement s'ils manquent ; l'échéance vient de la date clé dont le libellé contient limite / remise / dépôt / soumission). `TenderCriterion.position` garde l'ordre du règlement. **Un champ Pydantic nommé `date` doit être typé `dt.date`** : l'annotation est évaluée après l'affectation (`date | None = None` ⇒ `NoneType | NoneType`). Validé en réel : RC de 2 pages ⇒ 4 critères pondérés p. 2, 4 dates p. 1, 11 pièces, 4 conditions ; sur une pièce hors sujet le modèle ne rend rien (0 critère).
> - Frontend : `DossierSection` (dans l'onglet Analyse, après score et décision) porte les hooks ; `useTenderAnalysis` rend `null` sur 404 `analysis_missing` ; `JobProgress.summary(job)` dépend du type de job (sinon le dernier message). Les liens de la barre latérale sont en `prefetch={false}` (pages des phases suivantes ⇒ 404 préchargés en boucle). Les libellés IA varient d'un passage à l'autre (« Date limite de remise des offres » / « Remise des offres ») : les tests réels cherchent par regex.
> - Sources réelles : j360.info a des pages d'avis lisibles (`link_pattern /appels-d-offres/\d{6,}-`, acheteur masqué) ; datao.ma / marchefacile.ma sont rendus en JavaScript. Le secteur rendu par l'extracteur peut dépasser 128 caractères : `normalize()` borne tous les champs texte aux longueurs des colonnes (`LIMITS`, `clip`).
...
> **Écarts constatés à l'exécution (24/09/2026) — à respecter dans les phases suivantes :**
> - Exigences : un appel LLM **par pièce lisible** (`build_corpus([doc], max_chars=60_000)`) ; une pièce en échec est ignorée (log `requirements.document_failed`), les autres sont extraites. `merge_outputs` dédoublonne à `token_set_ratio ≥ 92` (la version obligatoire l'emporte) ; la ré-extraction rapproche à ≥ 95 et **conserve id, code et — si `manual_status` — statut et justification**, les exigences disparues sont supprimées. `TenderRequirement.manual_status` est posé par `PATCH /requirements/{id}` dès qu'un `status` ou une `justification` est envoyé : le moteur d'éligibilité ne revient jamais dessus. Migration `c16d7e8f9a0b_tender_requirements`.
> - Éligibilité : règles **par catégorie** (`EligibilityEngine.judge`) avec deux corrections venues du réel — (1) les mots trop génériques (`GENERIC_WORDS` : attestation, certificat, document, obligatoire, fournir, copie…) sont exclus du rapprochement exigence ↔ document, sinon « attestation CNSS » était satisfaite par « Attestation fiscale » et « visite obligatoire » par le tag « obligatoire » ; le document qui partage **le plus** de mots distinctifs l'emporte ; (2) les chiffres sont lus sur la forme **normalisée sans accents** (`_numbers` sur `norm_text`) et les nombres en toutes lettres comptent (« deux references » ⇒ 2). Un terme court (< 5 caractères) n'est reconnu qu'en mot entier (le flou inventait des liens). `evaluate(tender, answers_by_requirement)` : les réponses de l'utilisateur priment, les statuts manuels sont sautés, le résumé va dans `tender.extra["eligibility"]`, `ScoreService` relance le score avec le ratio, puis RB-003 (`on_mandatory_unmet`, journal en attendant les notifications de la Phase 11). `apply()` / `refresh()` sont réutilisés par la réponse à une question (re-jugement d'une seule exigence).
> - Questions : une par exigence `INFO_MANQUANTE`, `A_VERIFIER` **ou `NON_CONFORME`** (une non-conformité vient souvent d'un profil incomplet : attestation non déposée, expert non saisi — le plan ne citait que les deux premiers) qui n'a ni question ouverte ou ignorée, ni jugement déjà fondé sur une réponse, ni statut manuel. Un appel LLM par exigence (prompt `question_generate` v1 : exigence + verdict des règles + ce que l'entreprise possède dans la catégorie) ; en cas d'échec du modèle, question de repli construite depuis l'exigence — une information manquante n'est jamais sans question. Obligatoire ⇒ priorité `CRITIQUE`. Génération et évaluation sont des **jobs** (202), pas des appels synchrones : 18 exigences = 18 appels. `QuestionAnswer.text` est la propriété qui satisfait le contrat `AnswerLike` du moteur. Migration `2653402e22ea_questions`.
> - SQLAlchemy 2 : `Question(tender=…, requirement=…)` **n'entre pas en session** par le backref (`cascade_backrefs` supprimé) — `db.add()` explicite, sinon `SAWarning: Object of type <Question> not in session`. Un `Protocol` dont l'implémentation expose une `@property` doit déclarer la sienne (`id`/`text` en propriétés), et les dictionnaires de réponses se typent `Mapping[..., Sequence[...]]` (invariance de `list`).
> - Frontend : `lib/requirements.ts` porte les libellés et variantes de badge (statuts, catégories, priorités, preuves) ; `RequirementsSection` / `QuestionsSection` portent requêtes, jobs et toasts, les composants restent présentationnels (comme `DossierSection` en Phase 6). La correction manuelle d'un statut est derrière un crayon (un `<select>` par ligne dupliquait le libellé du badge dans le DOM et alourdissait le tableau). Les liens « preuves cliquables » attendent la Phase 8.5 (popover + routes `?open=`) : pour l'instant des puces titrées par nature de preuve. `JobProgress` : bilan de `evaluate_eligibility` et `generate_questions`, et libellé de progression par type de job (« Recherche en cours » était affiché pour tous depuis la Phase 6).
> - Réel (AO 27/2026, profil de dev quasi vide) : 18 exigences avec source `RC-27-2026.pdf`, éligibilité 11 %, 13 obligatoires non satisfaites, 18 questions précises (« votre attestation fiscale 2025 est expirée au 31/12/2025. Pouvez-vous fournir une attestation valide ? ») ; une réponse « Oui » passe l'exigence à `CONFORME` avec preuve `answer`, met à jour résumé et score, et la regénération ne duplique rien.
